...
module.exports = async (interaction, options) => {
	...
	await interaction.message.edit({ content: `${member.user.tag} has been muted.`, components: null }).catch(err => console.error(err));
	await interaction.editReply({ content: `${member.user.tag} has been muted.`, ephemeral: options.hidden }).catch(err => console.error(err));
